...
设备选择依据文件名缩小检索范围，未匹配的资料需手动选择；它**不是访问授权**。

文档可见范围由 `root` 逐份设置：默认「所有人可见」，也可改为「仅下列账号或用户组可见」。
受限文档对未授权账号**在检索、文档列表、原文访问、历史对话四条链路上都不可见**——
不是"藏起来"，而是根本检索不到；被撤销授权后，旧对话里引用过它的那一轮也会一并隐藏。
`root` 与 `kb_admin` 不受此限制（否则无法管理受限文档），管理员打开受限原文会记入审计。

**用户组**用于"一次授权给一批人"：`root` 在「用户组」页签里创建组并维护成员，之后在文档的
可见范围里勾选该组即可；组内成员即刻获得访问权，把某人移出组就等于撤销这批文档的访问权
（立即生效，且他此前引用过这些文档的对话会同步隐藏）。组只影响"谁能看什么"，
不参与检索范围，也不是按部门自动归类。

注意：默认仍是共享文档库。普通用户不能直接下载完整文件，但**未设限的短文档**可能完整落在
一个引用片段中，多次问答也可能逐步获知内容——因此需要保密的资料请务必设为受限，
不要依赖"不能下载"。
