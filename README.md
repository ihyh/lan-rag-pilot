...
文档可见范围由 `root` 逐份设置：默认「所有人可见」，也可改为「仅指定账号可见」。
...
注意：默认仍是共享文档库，**没有按部门/组隔离**。若需要按部门隔离，需在现有按用户授权
之上另行设计（见 [路线图](ROADMAP.md)）。普通用户不能直接下载完整文件，但**未设限的短文档**
可能完整落在一个引用片段中，多次问答也可能逐步获知内容——因此需要保密的资料请务必设为受限，
不要依赖"不能下载"。
