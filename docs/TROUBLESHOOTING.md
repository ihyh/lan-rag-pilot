...
| `parse_crashed` | 子进程没留下说明就退出（段错误，或被系统因内存不足杀掉） | 文件很可能损坏；**服务器未受影响**，其他功能可继续使用 |

要区分清楚：这四类都**不会**影响其他用户——解析在独立子进程里跑，坏文件只让这一次上传失败。如果现象是"整个服务都没响应了"，那要看 [运行慢](下文) 与内存，而不是解析隔离。
