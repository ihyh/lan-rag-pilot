...
文档列表里那一行的失败原因会写明具体原因；这几类与解析隔离有关，处理方向不同，别混着调参：

| 失败原因里的 code | 含义 | 该怎么办 |
|---|---|---|
| `parse_isolation_unavailable` | 无法建立解析子进程；系统为避免在主服务内处理不可信文件而拒绝本次操作 | 检查系统进程/句柄资源与服务运行权限。不要为了绕过错误而直接关闭隔离处理可疑文件 |
| `parse_timeout` | 解析超过 `RAG_PARSE_TIMEOUT_S`（默认 300 秒）被终止，进程树已一并杀掉 | 先确认这份文件本身是否损坏或异常庞大；文件正常再调大超时。**不要**因为一份坏文件就把超时调得极大 |
| `parse_memory` | 解析占用超过 `RAG_PARSE_MEMORY_MB`（默认 2048 MB）。**只有 Linux 会出现** | 同上：先怀疑文件；调大前确认服务器内存足够，且不会影响正在服务的其他用户 |
| `parse_failed` | 解析组件抛了异常，已转成可读信息（信息里带异常类型） | 文件很可能损坏或格式不兼容；把文件与这条信息一并反馈，便于定位是哪个解析组件 |
| `parse_crashed` | 子进程没留下说明就退出（段错误，或被系统因内存不足杀掉） | 文件很可能损坏；主服务仍在。若宿主机同时内存紧张，先检查系统资源，尤其是没有进程内存硬上限的 Windows |

要区分清楚：这些错误都只应让本次上传失败；隔离进程无法建立时系统会 fail-closed，不会退回主服务解析同一份不可信文件。如果现象是"整个服务都没响应了"，那要看 [运行慢](下文) 与内存，而不是解析隔离。

如果怀疑是隔离机制本身的问题，可临时设 `RAG_PARSE_ISOLATION=0` 回到同进程解析来对照；确认后请改回 `1`——关掉后一份坏文件就能中断全站服务。
