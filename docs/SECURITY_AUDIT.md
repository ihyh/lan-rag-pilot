...
- 向量索引遇到空/损坏 BLOB 会跳过，不因单条脏数据阻断启动；真实嵌入支持低相似度拒答阈值，并使用精确技术词补召回和近重复去除。
- 新对话可限定文档范围，前端提供 PLM、PLUSPRO、PLUS500 快捷选择；后端持久化并锁定该范围，避免后续轮次混入其它设备文档。
- `/api/health` 是存活探针，`/api/ready` 是模型就绪探针；Compose healthcheck 使用后者。

## 本轮本机复核证据

- 2026-09-09 在隔离临时目录运行完整 smoke：145 项通过、0 项失败，重启持久化通过；覆盖登录、角色权限、多格式上传、设备/文档范围、多轮对话、反馈、CSV、并发和模型异常。该 mock 结果不能代表真实答案质量。
...
1. Ubuntu 每日 02:30 备份 cron 已存在；仍须监控最近成功状态、把备份复制到独立受控存储，并完成一次新机器恢复演练。
2. 通过公司内网证书或反向代理启用 HTTPS，Compose 端口只绑定本机，设置 `RAG_COOKIE_SECURE=true`。
3. 更换 root 初始口令，配置高熵 `RAG_SECRET_KEY`，每个人使用独立账号；不得复用测试代码中的默认口令。
4. 防火墙只允许公司网段访问 443；不要把 8088 暴露到公网。
5. 用真实文档人工填写至少 30 条 `data/eval/questions.jsonl`；先记录固定 BGE + `qwen3:1.7b` 基线，不同时调整检索参数。
6. 先完成固定 IP/内部 DNS，再在另一台公司电脑和手机 Wi-Fi/VPN 上验证登录、问答、引用、反馈和 root 管理页。

## 有意保留的试点边界

- 单进程、SQLite、内存向量索引；超过约 5 万切片或出现并发瓶颈再迁移 PostgreSQL + pgvector。
- 逻辑角色为 root/kb_admin/user；为兼容旧 SQLite CHECK，`kb_admin` 在库内存为 `role='user'` 加 `is_kb_admin=1`。SSO、机器人、OCR、旧版 XLS 和 PPT/PPTX 未实现；XLSX 已支持。
