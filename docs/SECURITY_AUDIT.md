...
5. 用真实文档人工填写至少 30 条评测问题，再调 `RAG_MIN_RELEVANCE_SCORE`；当前没有自动生成的标准答案。
...
本审计只覆盖源码、配置模板和本机静态检查。Ubuntu 实机重建、cron 安装、DNS、HTTPS、Windows/公司防火墙、企业微信/飞书后台和真实 DeepSeek API 均未在本轮重新执行，不将其标记为已完成。
