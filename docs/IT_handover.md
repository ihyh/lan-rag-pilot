...
隔离层新增的失败原因按五类区分，code 会写进文档的失败原因字段，便于管理员分辨"文件坏了"和"服务器资源不够"：

| code | 含义 | 处理方向 |
|---|---|---|
| `parse_isolation_unavailable` | 无法建立解析子进程，已拒绝在主服务内解析 | 检查系统进程/句柄资源和服务运行权限；不要关闭隔离后直接重试可疑文件 |
...
| `parse_crashed` | 子进程没留下说明就退出（段错误/被 OOM 杀掉） | 文件很可能损坏；主服务仍在。若宿主机内存也紧张，先检查系统资源 |

解析器自己抛出的 `ParseError`（例如"扫描件无文本层"）**原样透传**，message 与 code 与不隔离时逐字一致——隔离不应改变正常业务的错误语义。

## 性能观测：把"为什么慢"变成一眼可判

历史上"为什么很慢且没有回答"这类问题只能靠人工取证：查进程内存、`ollama ps`、手工 curl 测吞吐。原因之一是 `chats` 表只记录**模型调用**耗时，检索段完全不可见。现在每轮问答分阶段记录，管理页概览直接给出统计：

| 记录项 | 含义 |
|---|---|
| `latency_ms` | 模型调用耗时。**包含模型冷加载**（闲置卸载后再问，纯 CPU 实测多约 25 秒）；不含并发排队或完整 HTTP 请求耗时 |
| `retrieval_ms` | 检索段耗时：构造检索问题 → 选出最终来源。只依赖本机嵌入模型与内存索引 |

对照同一次成功问答的两段耗时可判断已记录阶段的瓶颈：检索几百毫秒而模型调用几十秒，应优先查生成模型；检索到秒级，应检查嵌入或索引。若两段都快但用户仍等待很久，还需检查并发排队和网络。概览的 `latency` 块给出最近 500 轮的 p50 / p95 / 最长值、样本量、统计截至时间与拒答、失败计数；不同阶段的统计样本可能不同，不宜直接相减当成请求总耗时。

统计口径有三点必须说清楚，否则数字会被误读：

- 模型耗时只统计**成功作答且有模型调用耗时**的轮次。拒答（知识库为空、无匹配、无权访问）也写 `chats` 且 `status='ok'`，但它没有模型调用、`latency_ms` 为空；若只按 `status='ok'` 取样本，等于把无耗时的行混进统计。
- 两段耗时的样本**刻意不同**：模型耗时只含成功作答（失败轮次的耗时没有可比性，混进去会把"一次断链"误读成"系统变慢"）；检索耗时覆盖所有发生过检索的轮次（含无命中与随后生成失败的），因为检索成本与后续是否失败无关。
- 百分位用**最近秩**，样本量小时比插值法保守；窗口是**最近 N 轮**而不是固定时长，因此查询成本与数据库大小无关。

审计明细里同样记录这两项耗时，便于事后核对某一次具体问答。行为测试见 `tests/latency_stats_check.py`（含口径断言与旧库迁移）。
