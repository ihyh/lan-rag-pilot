...
| 记录项 | 含义 |
|---|---|
| `latency_ms` | 模型调用耗时。**包含模型冷加载**（闲置卸载后再问，纯 CPU 实测多约 25 秒） |
| `retrieval_ms` | 检索段耗时：构造检索问题 → 选出最终来源。只依赖本机嵌入模型与内存索引 |

两者在同一行上对比即可定位瓶颈：检索几百毫秒而总耗时几十秒 = 生成是瓶颈（纯 CPU 部署的预期表现）；检索也到秒级 = 索引或嵌入有问题。概览的 `latency` 块给出最近 500 轮的 p50 / p95 / 最长值与拒答、失败计数，并直接用一句话指出瓶颈方向。

统计口径有三点必须说清楚，否则数字会被误读：

- 只统计**真正发起过模型调用**的轮次。拒答（知识库为空、无匹配、无权访问）也写 `chats` 且 `status='ok'`，但它没有模型调用、`latency_ms` 为空；若按 `status='ok'` 取样本，等于把无耗时的行混进统计。
- 两段耗时的样本**刻意不同**：总耗时只含成功作答（失败轮次的耗时没有可比性，混进去会把"一次断链"误读成"系统变慢"）；检索耗时覆盖所有发生过检索的轮次（含随后生成失败的），因为检索成本与后续是否失败无关。
