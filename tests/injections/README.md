...
快速检查锚点：`python tests/injections/check_anchors.py`（CI 里只跑这一步，代价低）。只跑某一组：`python tests/injections/run_injections.py --group eval`（可选 `probe`、`eval`、`isolation`、`latency`、`retrieval`、`chunking`、`encoding`）。`isolation` 的 Windows `taskkill` 注入只在 Windows 上运行。
