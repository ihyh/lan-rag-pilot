  ...
  async function loadDocs() {
    ...
    try {
      ...
      var rows = data.items.map(function (d) {
        ...
        return h('tr', {}, [name, cell(fmtBytes(d.size_bytes)), cell(d.num_chunks), h('td', {}, [state]), cell(d.knowledge_base_names || '默认知识库'), cell(d.uploaded_by_name), cell(fmtTime(d.created_at)), actions]);
      });
      body.appendChild(table(['文件', '大小', '切片', '状态', '知识库', '上传者', '上传时间', '操作'], rows));
