...
脚本默认保证相邻查询至少间隔 6.1 秒，对应服务默认每分钟 10 次的限流；模型调用本身超过该时长时不会额外等待。若独立验收实例使用不同限流，可通过 `--min-interval` 调整（必须是有限且不小于 0 的秒数），只有确认实例不限流时才设为 0。不要关闭生产限流；429、无效 JSON 和畸形查询响应都会保留为该题的错误记录并继续后续题目，不能将失败题排除后宣称通过。登录失败仍会立即终止整次评测。
